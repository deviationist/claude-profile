...
```sh
zsh tools/generate-readme-svg.zsh    # → assets/{status,selector}-<hash>.svg + README refs
```
...
that captures the row list the real picker feeds it) and runs the tool
unmodified. Rerun it whenever the status layout or the selector changes; commit
